...
public class AbstractHandlerUnitTests
{
    ...
    [Fact]
    public async Task Deve_Enviar_Uma_Lista_De_Notificacoes_Texto_Caso_A_Condicao_Atenda_Todos_Os_Requisitos()
    {
        _mockMediator.Setup(s => s.Publish(It.IsAny<AppNotification>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

        var response = await _handler.NotificationHasBeenPublishedAsync(true, 400, new List<string> { "Message", "Message" });
    ...
    [Fact]
    public async Task Nao_Deve_Enviar_Uma_Lista_De_Notificacoes_Texto_Pois_A_Condicao_Nao_Atende_Todos_Os_Requisitos()
    {
        _mockMediator.Setup(s => s.Publish(It.IsAny<AppNotification>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

        var response = await _handler.NotificationHasBeenPublishedAsync(false, 400, new List<string> { });
    ...
    [Fact]
    public async Task Deve_Enviar_Uma_Lista_De_Notificacoes_ValidationFailure_Caso_A_Condicao_Atenda_Todos_Os_Requisitos()
    {
        _mockMediator.Setup(s => s.Publish(It.IsAny<AppNotification>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
    ...
    [Fact]
    public async Task Nao_Deve_Enviar_Uma_Lista_De_Notificacoes_ValidationFailure_Pois_A_Condicao_Nao_Atende_Todos_Os_Requisitos()
    {
        _mockMediator.Setup(s => s.Publish(It.IsAny<AppNotification>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
